...
工作流仅为 Gateway 从固定的 VPS-Toolkit Release 下载并校验 Reporter APK。Gateway 预设源地址模式和以 Router 为真实 DNS 的解析路径，专用源地址留空，上报保持禁用。部署时先检查地址冲突、持久化本机源地址、配置主路由 WAN-only 分流和 OpenClash 仅本机 OUTPUT 绕过，再在 LuCI 配置上报对象、密钥及所选 WAN。普通 Worker 提交通道继续使用正常网络路径；源地址仅用于出口查询与官方直连。详见 [初始化说明](./docs/README.md)。
